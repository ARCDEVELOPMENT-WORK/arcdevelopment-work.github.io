...
const TOKEN = '7206469247:AAHWlHR4LDMmubyjag9QZEClF0RJpCmBDSE'; // Replace with your bot token
...
const LOGIN_URL = "https://free-gsscript-arc.onrender.com/Login.php";

// Start PHP built-in server on a random port between 10000 and 11000
const { spawn } = require('child_process');
const minPort = 10000;
const maxPort = 11000;
let phpPort = Math.floor(Math.random() * (maxPort - minPort + 1)) + minPort;
let LOCAL_PHP_BASE_URL = `http://localhost:${phpPort}/`;

const phpServer = spawn('php', ['-S', `0.0.0.0:${phpPort}`, '-t', __dirname]);

phpServer.stdout.on('data', (data) => {
  const output = data.toString();
  console.log(`PHP Server: ${output}`);

  // Detect port change from output like: "PHP 7.4.33 Development Server (http://0.0.0.0:10676) started"
  const portMatch = output.match(/http:\/\/0\.0\.0\.0:(\d+)/);
  if (portMatch) {
    const newPort = parseInt(portMatch[1], 10);
    if (newPort !== phpPort) {
      phpPort = newPort;
      LOCAL_PHP_BASE_URL = `http://localhost:${phpPort}/`;
      console.log(`==> Detected a new open port HTTP:${phpPort}`);
    }
  }
});
...
phpServer.stderr.on('data', (data) => {
  const output = data.toString();
  console.error(`PHP Server Error: ${output}`);

  // Detect port change from error output as well
  const portMatch = output.match(/http:\/\/0\.0\.0\.0:(\d+)/);
  if (portMatch) {
    const newPort = parseInt(portMatch[1], 10);
    if (newPort !== phpPort) {
...
// Use LOCAL_PHP_BASE_URL in register or other functions that communicate with local PHP server
...
bot.on('message', async (msg) => {
  ...
  if (msg.text && msg.text.startsWith('/login')) {
    ...
    if (parts.length !== 3) {
      bot.sendMessage(chatId, 'Usage: /login <username> <password>');
      return;
    }
    ...
    if (loginResponse) {
      bot.sendMessage(chatId, `Login response:\n${JSON.stringify(loginResponse)}`);
    } else {
